...
int blinkCounter     = 0;	// statusLed blink timer
int deadPowerCounter = 0;
...
void setup() {
  ...
  if (DEBUG) {
    Serial.println("Ready! Press button to wake.");
  }
...
void loop() {
  ...
  if (sleepSignal == 1) {
    ...
    statusLedCtrl(1);
    if (DEBUG) {
      Serial.println("RPiPowerSwitch is ON");
    }
    delay(500);
  ...
  if (isLowPower) {
    ...
    if (blinkCounter == 30000) {
      ...
      blinkCounter = 0;
      if (BATTERY_DEBUG) {
        Serial.println("  Battery  power is low");
      }
    }
  }
}
...
void wake_ISR() {
  ...
  if (isReleased == 1) {           // when release the button, change the signal
    releaseTime = millis();
    if (DEBUG) {
      Serial.println("  Button releaseed");
    }
    if ((releaseTime - pressTime) > LONGPRESS) { // long pressed
      sleepSignal = 2;
      if (DEBUG) {
        Serial.println("    Type: long press");
      }
    }
    else {                                     // short pressed
      ...
      if (DEBUG) {
        Serial.println("    Type: short press");
      }
    }
    if (DEBUG) {
...
void checkBattery() {
  ...
  */
  ...
  if (batteryVoltage < 6) {
    deadPowerCounter ++;
    if (BATTERY_DEBUG) {
      Serial.println("  System power is less then 6V, and go to sleep");
    }
    if (deadPowerCounter > 3) {
      for (int i = 0; i < 26; i++) {
  ...
  // raspi will tell arduino powerType, every power-type has different alarm gate
    ...
    deadPowerCounter = 0;
    if (batteryVoltage < alarm_gate[powerType])
      isLowPower = true;
    else
      isLowPower = false;
  }
...
void doSleep() {
  double poweroffDelayBegin = 0;
  double poweroffDelayEnd = 0;
  if (DEBUG) {
  ...
  do {
    if (DEBUG) Serial.println(" power Off Led Breath Up");
    powerOffLedBreathUp();        // led breath effect
    if (DEBUG) Serial.println(" power Off Led Breath Down");
  ...
  } while ((poweroffDelayEnd - poweroffDelayBegin) < 4000);
  if (DEBUG) Serial.print("  Raspberry Pi is off");
  rpiSwitchCtrl(RPIPOWEROFF);
  if (DEBUG) {
    Serial.println("  Turn off power of Raspberry Pi  ");
